...
const std::string outDirRandom = "outputRandom/";

int myrandom(int i){return std::rand()%i;}

int createRandomStrings(const std::string inFileName, const std::string inFileName2, const std::string outName, bool doAppend, int numberOutputLines)
{
  std::ifstream file;
  std::string str; 
  ...
  for(int iter = 0; iter < inStr1Size; iter++){
	...
	if(inStr2_p->at(iter2).find(inStr1Words_p->at(wordIter)) != std::string::npos){
	  ...
	  std::vector<std::string>* newStrWords_p = new std::vector<std::string>;
	  getWordsFromSentence(newStr, newStrWords_p);
	  std::random_shuffle(newStrWords_p->begin(), newStrWords_p->end(), myrandom);
	  newStr = "";
	  for(int newStrWordIter = 0; newStrWordIter < (int)newStrWords_p->size(); newStrWordIter++){
	    if(newStrWordIter < (int)newStrWords_p->size() - 1) newStr = newStr + newStrWords_p->at(newStrWordIter) + " ";
	    else newStr = newStr + newStrWords_p->at(newStrWordIter);
	  }
  ...
  if(checkMakeDir(outDirRandom)){
    ...
    std::cout << "Error making dir \'" << outDirRandom << "\'. File \'" << tempOutName << "\' not written." << std::endl;
  }
...
int runCreateRandomStrings(const std::string inFileName, const std::string inFileName2, int numberOutputLines)
{
  std::string outName;
  ...
  int retVal = createRandomStrings(inFileName, inFileName2, outName, false, tempNumberOutputLines1);
  if(inFileName.size() != inFileName2.size() || inFileName.find(inFileName2) == std::string::npos) retVal = createRandomStrings(inFileName2, inFileName, outName, true, tempNumberOutputLines2);
...
{
  if(argc != 4){
    std::cout << "Usage: createRandomStrings.exe <inputFile> <inputFile2> <numberOfOutputLines, -1 if max>" << std::endl;
  ...
  int result = runCreateRandomStrings(argv[1], argv[2], std::stoi(argv[3]));
